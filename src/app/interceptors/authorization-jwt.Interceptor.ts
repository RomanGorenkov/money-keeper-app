...
export class AuthorizationJwtInterceptor implements HttpInterceptor {
  intercept(req: HttpRequest<any>, next: HttpHandler) {
    ...
    if (token) {
      newReq = newReq.clone({
        headers: newReq.headers.append(httpHeader.name.authorization, `${httpHeader.value.authorization}${token}`),
      })
    }

    if (!newReq.headers.has(httpHeader.name.xImg)) {
      newReq = newReq.clone({
        headers: newReq.headers.append(httpHeader.name.contentType, httpHeader.value.json),
      })
    }
