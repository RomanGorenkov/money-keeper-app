...
export class CostService {
  ...
  constructor(private costApiService: CostApiService, private storageService: StorageService) {}
  ...
  addCost(costDto: CostDto) {
    return this.costApiService.addCost(costDto).pipe(tap(() => this.addCostInCurrentCostList(costDto)))
  }

  changeCostList(startDate = this.getStartDate(), endDate = this.getEndDate()) {
    this.costApiService.getCurrentAllUserCosts(startDate, endDate).subscribe(costList => this.setCostList(costList))
  }
  ...
  getCostsColors() {
    const costColorListFromStorage = this.storageService.getStorageElement<CostCategoryColorList>(
      storageKeys.costColors
    )
  ...
  setTodayCosts() {
    this.costApiService.getTodayAllUserCosts().subscribe(costList => this.setCostList(costList))
  }
