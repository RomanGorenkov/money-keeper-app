...
export class CostCategoryService {
  ...
  constructor(private costService: CostService, private storageService: StorageService, private http: HttpClient) {}
  ...
  addNewUserCostCategory(costCategoryData) {
    ...
    return this.http.post<string>(`${environment.serverUrl}/${apiUrls.addNewUserCostCategory}`, costCategoryData, {
      headers,
    })
  }

  setCostCategoryListByNameList(categoryNameList: string[]) {
    const customCategoryListFromStorage = this.storageService.getStorageElement<CostItemConfig[]>(
      storageKeys.customCategoryList
    )

    this.costCategoryList.next(
