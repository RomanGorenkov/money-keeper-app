...
export class DatePickerComponent {
  ...
  constructor(private dateService: DateService, private dialogService: DialogService) {}

  openDateSwitcher() {
    this.dialogService.open(
      DateSwitcherComponent,
      { data: { dateSwitcherName: 'timeInterval' } },
      dialogOverlayColor.clear
    )
  }
  ...
  changeDate(timeInterval: number, direction: number) {
    if (direction < 0) {
      this.dateService.changeCurrentDateByCustomDate(
        this.dateService.getStartDate() - timeInterval,
        this.dateService.getStartDate()
      )
    } else {
      this.dateService.changeCurrentDateByCustomDate(
        this.dateService.getStartDate() + timeInterval,
        this.dateService.getStartDate() + 2 * timeInterval
      )
    }
  }
}
